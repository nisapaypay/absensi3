...
<?php 
	...
	if(isset($_POST['btnSimpan']))
	{
		...
		if (mysqli_num_rows($cekdata) > 0) {
			...
			# code...
			$simpan = mysqli_query($konek, "insert into siswa(nomorkartu, nama, nis, kelas, alamat)values('$nomorkartu', '$nama', '$nis', '$kelas', '$alamat')");
			echo "
				<script>
					alert('Tersimpan');
					location.replace('datasiswa.php');
...
<body>
	...
	<div class="container-fluid">
		...
		<form method="POST">
			...
			<div class="form-group">
				<label>Nama</label>
				<Input type="text" name="nama" id="nama" placeholder="nama siswa" class="form-control" style="width: 400px" required>
			</div>
			<div class="form-group">
				<label>NIS</label>
				<Input type="text" name="nis" id="nis" placeholder="nis" class="form-control" style="width: 400px" required>
			</div>
			<div class="form-group">
				<label>Kelas</label>
				<select class="form-control" name="kelas" style="width: 400px" required>
					<option>Pilih</option>
			...
			<div class="form-group">
				<label>Alamat</label>
				<textarea class="form-control" name="alamat" id="alamat" placeholder="alamat" style="width: 400px" required></textarea>
			</div>
